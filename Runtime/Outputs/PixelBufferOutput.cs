...
namespace NatML.Devices.Outputs {
    ...
    public sealed class PixelBufferOutput : CameraOutput {
        ...
        public unsafe void Update (CameraImage cameraImage, ConversionOptions options) {
            // Create
            var bufferSize = cameraImage.width * cameraImage.height * 4;
            ...
            if (!tempBuffer.IsCreated)
                tempBuffer = new NativeArray<byte>(cameraImage.width * cameraImage.height * 4, Allocator.Persistent);
            // Check options
