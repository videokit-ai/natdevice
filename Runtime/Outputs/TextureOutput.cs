...
namespace NatML.Devices.Outputs {
    ...
    public sealed class TextureOutput : CameraOutput {
        ...
        #region --DEPRECATED--
        [Obsolete(@"Deprecated in NatDevice 1.3.2. Use the `NextFrame` method instead.", false)]
        public Task textureCreated => NextFrame();

        [Obsolete(@"Deprecated in NatDevice 1.3.2. Use `OnFrame` event instead.", false)]
        public event Action onFrame;

        [Obsolete(@"Deprecated in NatDevice 1.3.1. Use `NextFrame` method instead.", false)]
        public TaskAwaiter<Texture2D> GetAwaiter () => NextFrame().GetAwaiter();
        #endregion
    }
}