...
namespace NatML.Devices.Outputs {
    ...
    public sealed class RenderTextureOutput : CameraOutput {
        ...
        public void Update (CameraImage cameraImage, ConversionOptions options) {
            // Create
            var imageBufferSize = cameraImage.width * cameraImage.height * 4 + 16 * cameraImage.height; // Overallocate in case of padding
            ...
            options ??= new ConversionOptions {
                ...
                mirror = cameraImage.verticallyMirrored
            };
            // Convert
            var convertedTextureDescriptor = new RenderTextureDescriptor(cameraImage.width, cameraImage.height, RenderTextureFormat.ARGB32, 0);
            ...
            var conversionKernel = conversionKernelMap[cameraImage.format];
            UploadImage(cameraImage, conversionBuffer, conversionOffset, conversionStride);
            ...
            shader.Dispatch(conversionKernel, Mathf.CeilToInt((float)cameraImage.width / gx), Mathf.CeilToInt((float)cameraImage.height / gy), 1);
            ...
            var width = portrait ? cameraImage.height : cameraImage.width;
            var height = portrait ? cameraImage.width : cameraImage.height;
            // Create
