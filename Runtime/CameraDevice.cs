...
namespace NatML.Devices {
    ...
    public sealed class CameraDevice : IMediaDevice<CameraImage> {
        ...
        /// <summary>
        /// Start the camera preview along with camera depth.
        /// </summary>
        /// <param name="imageHandler">Delegate to receive preview image frames.</param>
        /// <param name="depthHandler">Delegate to receive preview depth frames.</param>
        internal void StartRunning (Action<CameraImage> imageHandler, Action<CameraImage> depthHandler) {
        ...
        public void CapturePhoto (Action<CameraImage> handler) {
            ...
            if (Application.platform == RuntimePlatform.WindowsPlayer || Application.platform == RuntimePlatform.WindowsEditor)
                throw new NotImplementedException(@"Photo capture is currently not supported on Windows");
            // Capture
            var syncContext = SynchronizationContext.Current; // send to main thread if available
            var unityThreadID = Thread.CurrentThread.ManagedThreadId;
            SendOrPostCallback callback = arg => {
