...
namespace NatML.Devices {
    ...
    using Internal;
    using AppTokenStatus = Internal.NatDevice.AppTokenStatus;
    ...
    public sealed class MediaDeviceQuery : IReadOnlyList<IMediaDevice> {
        ...
        private static void OnInitialize () {
            var status = NatDevice.SetAppToken(NatDeviceSettings.Instance.Token);
        ...
        private static IEnumerable<AudioDevice> AudioDevices () {
            ...
            NatDevice.AudioDevices(devices, ref count);
        ...
        private static IEnumerable<CameraDevice> CameraDevices () {
            var devices = new IntPtr[1 << 6];
            var count = devices.Length;
            NatDevice.CameraDevices(devices, ref count);
            for (var i = 0; i < count; ++i)
                yield return new CameraDevice(devices[i]);
        }

        private static string MessageForTokenStatus (NatDevice.AppTokenStatus status) => status switch {
            AppTokenStatus.Invalid      => @"NatDevice Error: Failed to validate app token. Please check your NatML access key.",
            AppTokenStatus.HubMissing   => @"NatDevice Error: Failed to set app token because NatMLHub native library was not found.",
            AppTokenStatus.HubInvalid   => @"NatDevice Error: Failed to set app token because NatMLHub native library is invalid.",
            _                           => null,
        };
