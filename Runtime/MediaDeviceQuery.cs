...
namespace NatML.Devices {
    ...
    public sealed class MediaDeviceQuery : IReadOnlyList<IMediaDevice> {
        ...
        public MediaDeviceQuery (Predicate<IMediaDevice> filter = null, int capacity = int.MaxValue) {
            // Check filter
            var filterFn = filter != null ? new Func<IMediaDevice, bool>(filter) : (_ => true);
            // Get devices
            var devices = new List<IMediaDevice>();
            devices.AddRange(GetAudioDevices());
            devices.AddRange(GetCameraDevices());
            // Filter
            this.devices = devices.Where(filterFn).Take(capacity).ToArray();
            this.index = 0;
        }
        ...
        private static IEnumerable<CameraDevice> GetCameraDevices () {
            var devices = new IntPtr[1 << 6];
