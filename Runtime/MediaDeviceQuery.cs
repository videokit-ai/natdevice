...
namespace NatML.Devices {
    ...
    public sealed class MediaDeviceQuery : IReadOnlyList<IMediaDevice> {
        ...
        public MediaDeviceQuery (
            Predicate<IMediaDevice> filter = null,
            int capacity = int.MaxValue,
            bool configureAudioSession = true
        ) : this(DiscoverDevices(filter, capacity, configureAudioSession)) { }
        ...
        private static IEnumerable<IMediaDevice> DiscoverDevices (Predicate<IMediaDevice> filter, int capacity, bool configureAudioSession) {
            var filterFn = filter != null ? new Func<IMediaDevice, bool>(filter) : (_ => true);
            var devices = new List<IMediaDevice>();
            devices.AddRange(DiscoverAudioDevices(configureAudioSession));
            devices.AddRange(DiscoverCameraDevices());
            return devices.Where(filterFn).OrderBy(ComputeDeviceOrder).Take(capacity);
        }

        private static IEnumerable<AudioDevice> DiscoverAudioDevices (bool configureAudioSession) {
